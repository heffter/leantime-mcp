...
- **Leantime returns `429 Too Many Requests` under burst load.** Leantime ships a per-minute rate limiter that fires when an MCP client (or an AI driving one) makes many calls in quick succession. The MCP client now retries automatically on 429 with exponential backoff (~1s, 2s, 4s, 8s, 16s) and 25% jitter, honoring a `Retry-After` response header if Leantime sends one. Caps and counts are tunable via env vars on the container: `LEANTIME_MAX_RETRIES` (default 5), `LEANTIME_BACKOFF_BASE` (default 1.0s), `LEANTIME_BACKOFF_CAP` (default 30s). If a tool call takes longer than usual, that's the retry waiting Leantime out. Persistent 429s past the retry budget surface as `httpx.HTTPStatusError` to the caller.
