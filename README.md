...
- **`assignedTo` is renamed to `editorId` on the wire** for `create_ticket`, `update_ticket`, and `upsert_subtask`. Leantime's `addTicket` and `updateTicket` services read `$values['editorId']`, NOT `$values['assignedTo']`, so the previous version silently dropped the assignee. The Pythonic `assignedTo` parameter on the MCP tools is preserved (no migration cost for callers); the translation happens in the client. `assignedTo=0` maps to the empty-string "unassign" sentinel, matching the `milestone_id=0` / `sprint_id=0` detach convention.
